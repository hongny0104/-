...
# 심사기준 점수표
기준점수 = {
    "5억 미만": {"사업수행능력": 35.0, "지역업체참여": 8.0, "경영상태": 2.0, "입찰가격": 55.0, "기술자보유": -10.0, "계약질서": -1.0},
    "10억 미만": {"사업수행능력": 45.0, "지역업체참여": 8.0, "경영상태": 2.0, "입찰가격": 45.0, "기술자보유": -10.0, "계약질서": -1.0},
    "10억 이상": {"사업수행능력": 65.0, "지역업체참여": 8.0, "경영상태": 2.0, "입찰가격": 22.0, "기술자보유": -10.0, "계약질서": -1.0},
}
...
with col1:
    점_사업 = st.number_input(
        "사업수행능력",
        min_value=0.0,
        max_value=float(현재기준["사업수행능력"]),
        step=0.5,
        format="%.1f"
    )
    점_지역 = st.number_input(
        "지역업체참여도",
        min_value=0.0,
        max_value=float(현재기준["지역업체참여"]),
        step=0.5,
        format="%.1f"
    )
with col2:
    점_경영상태 = st.number_input(
        "경영상태",
        min_value=0.0,
        max_value=float(현재기준["경영상태"]),
        step=0.5,
        format="%.1f"
    )
with col3:
    점_기술자 = st.number_input(
        "기술자보유 감점 (없으면 0)",
        min_value=float(현재기준["기술자보유"]),
        max_value=0.0,
        step=1.0,
        format="%d"
    )
    점_계약질서 = st.number_input(
        "계약질서 감점 (없으면 0)",
        min_value=float(현재기준["계약질서"]),
        max_value=0.0,
        step=1.0,
        format="%d"
    )
...
선택입찰가 = st.selectbox(
    ...
    [int(str(x).replace(",", "")) for x in 예정가격표_df["입찰가격"]]
)
...
st.markdown(f"- **선택 입찰가격 점수**: {round(선택가격점수)} / {현재기준['입찰가격']}")
st.markdown(f"- **선택 총점**: {round(선택총점)} / 100")
