...
st.dataframe(예정가격표_df)

# -------------------------------
# 선택 입찰가 분석
st.header("4. 입찰가격 선택 시 자동 분석")

선택입찰가 = st.selectbox("입찰가격 선택 (예정가격 시뮬레이션 참고)", [int(str(x).replace(",", "")) for x in 예정가격표_df["입찰가격"]])

# 선택된 입찰가 기준 점수계산
선택입찰률 = 선택입찰가 / 예정가격_평균 if 예정가격_평균 > 0 else 0
입찰률차이 = abs(88 - 선택입찰률 * 100)

# 입찰가격 점수 계산 로직
if 사업규모 == "5억 미만":
    선택가격점수 = max(0, 60 - 4 * 입찰률차이)
elif 사업규모 == "10억 미만":
    선택가격점수 = max(0, 50 - 2 * 입찰률차이)
else:
    선택가격점수 = max(0, 30 - 1 * 입찰률차이)
