...
예정가격표_df['예정가격'] = 예정가격표_df['예정가격'].map(lambda x: f"{x:,.0f}")
예정가격표_df['입찰가격'] = (예정가격표_df['예정가격'].str.replace(",", "").astype(int) * 낙찰입찰률).round().astype(int)
예정가격표_df['입찰가격'] = 예정가격표_df['입찰가격'].map(lambda x: f"{x:,.0f}")
st.write(예정가격표_df)

st.subheader("입찰결과 분석")
st.markdown(f"- **예정가격(평균)**: `{예정가격_평균:,.0f}원`")
st.markdown(f"- **입찰가격** (× {낙찰입찰률}): `{입찰가격:,.0f}원`")
...
선택입찰가 = st.selectbox("입찰가격 선택 (예정가격 시뮬레이션 참고)", 예정가격표_df["입찰가격"].tolist())
...
st.markdown(f"- **선택 입찰가격**: `{선택입찰가:,.0f}원`")
st.markdown(f"- **선택 입찰률**: `{선택입찰률:.4f}`")
st.markdown(f"- **선택 입찰가격 점수**: `{선택가격점수:.2f}` / {현재기준['입찰가격']}")
st.markdown(f"- **선택 총점**: `{선택총점:.2f}` / 100")

if 선택총점 >= 기술점수_최소:
    st.success(f"✅ 선택한 입찰가 기준으로 총점 {선택총점:.2f}점 → 낙찰 가능성 높음")
else:
    st.warning(f"⚠️ 선택한 입찰가 기준으로 총점 {선택총점:.2f}점 → 낙찰 가능성 낮음")
