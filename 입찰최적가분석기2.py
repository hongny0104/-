...
st.markdown(f"- **기술점수 총합**: {점_사업 + 점_지역 + 점_경영상태 + 점_입찰가격 + 점_기술자 + 점_계약질서:.2f}")
...
선택가격점수 = max(0, 현재기준["입찰가격"] * ((1 - 선택입찰률) / (1 - 낙찰입찰률))) if 선택입찰률 >= 낙찰입찰률 else 0
...
# -------------------------------
# 📌 사용자가 입찰가를 직접 입력하여 분석
st.subheader("입찰가격 직접 입력 시 자동 분석")

입력입찰가 = st.number_input("직접 입력할 입찰가격 (₩)", min_value=1000000, step=100000)

입력입찰률 = 입력입찰가 / 예정가격_평균 if 예정가격_평균 > 0 else 0
입력가격점수 = max(0, 현재기준["입찰가격"] * ((1 - 입력입찰률) / (1 - 낙찰입찰률))) if 입력입찰률 >= 낙찰입찰률 else 0
입력총점 = 점_사업 + 점_지역 + 점_경영상태 + 점_기술자 + 점_계약질서 + 입력가격점수

st.markdown(f"- **입력 입찰가격**: `{입력입찰가:,}원`")
st.markdown(f"- **입력 입찰률**: `{입력입찰률:.4f}`")
st.markdown(f"- **입력 입찰가격 점수**: `{입력가격점수:.2f}` / {현재기준['입찰가격']}")
st.markdown(f"- **입력 총점**: `{입력총점:.2f}` / 100")

if 입력총점 < 기준점수:
    st.error(f"❌ 총점 {입력총점:.2f}점은 기준점수 {기준점수}점 미만으로 낙찰 불가")
else:
    차이 = abs(입력총점 - 기준점수)
    st.success(f"✅ 총점 {입력총점:.2f}점 → 기준점수 {기준점수}점과의 차이: {차이:.2f}점")
    st.info(f"📌 {낙찰조건설명}")
